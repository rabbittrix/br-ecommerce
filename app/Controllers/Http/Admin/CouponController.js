...
class CouponController {
  ...
  async store ({ request, response, transform }) {
    ...
    try {
      ...
      return response.status(400).send({
        message: 'Error do not create your coupon!'
      })
    }
  }
  ...
  async update ({ params: {id}, request, response, transform }) {
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({
        message: 'Error do not update your coupon!'
      })
    }
  }
  ...
  async destroy ({ params: {id}, request, response }) {
    ...
    try {
      ...
      await coupon.delete().detach(trx)
      await trx.commit()
      return response.status(204).send({
        message: 'Success your coupon delete!'
      })
    } catch (error) {
      await trx.rollback()
      return response.status(400).send({
        message: 'Error do not delete your coupon!'
      })
    }
  }
}
